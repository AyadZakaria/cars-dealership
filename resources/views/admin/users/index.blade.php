...
<div x-data="userCrudModals()" x-init="init()">
    <div class="flex justify-between items-center mb-6">
        ...
        <button @click="openCreate = true" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 focus:outline-none">Add User</button>
    </div>
    <!-- Create User Modal -->
    ...
    <div x-show="openView" class="fixed inset-0 flex items-center justify-center z-50" style="display: none;" x-cloak>
        ...
        <div class="bg-white rounded-lg shadow-lg w-full max-w-md z-10 p-6">
            ...
            <div class="mb-2"><span class="font-semibold">Email:</span> <span x-text="selectedUser.email"></span></div>
            <div class="mb-2"><span class="font-semibold">Admin:</span> <span x-text="selectedUser.is_admin ? 'Yes' : 'No'"></span></div>
        ...
        <div class="bg-white rounded-lg shadow-lg w-full max-w-md z-10 p-6">
            ...
            <form :action="'/admin/users/' + selectedUser.id" method="POST" @submit="fixCheckbox">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="edit_name" class="w-full border rounded px-3 py-2" x-model="selectedUser.name" required>
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 mb-1" for="edit_email">Email</label>
                    <input type="email" name="email" id="edit_email" class="w-full border rounded px-3 py-2" x-model="selectedUser.email" required>
                </div>
                <div class="mb-4 flex items-center">
                    <input type="hidden" name="is_admin" value="0">
                    <input type="checkbox" name="is_admin" id="edit_is_admin" class="mr-2" x-model="selectedUser.is_admin" :checked="selectedUser.is_admin">
