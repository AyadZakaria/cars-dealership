            ...
            @forelse($cars as $car)
                <div
                    ...
                    <div class="p-5 flex-1 flex flex-col">
                        <h2 class="text-xl font-semibold mb-1 text-gray-900">{{ $car->brand }} {{ $car->model }}
                        </h2>
                        ...
                        <div class="flex flex-col gap-2 mt-4">
                            @php
                                $isForSale = $car->availability === 'for_sale';
                                $isForRent = $car->availability === 'for_rent';
                            @endphp

                            @if ($isForSale && $car->in_service)
                                <div>
                                    <span
                                        class="inline-block px-2 py-0.5 bg-blue-100 text-blue-700 rounded text-xs font-medium">For
                                        Sale</span>
                                    <div class="text-gray-900 text-base">Price: <span
                            ...
                            @if ($isForRent && $car->in_service)
                                <div>
                                    <span
                                        class="inline-block px-2 py-0.5 bg-yellow-100 text-yellow-500 rounded text-xs font-medium">For
                                        Rent</span>
                                    <div class="text-gray-900 text-base"><span
                                            class="font-bold">${{ number_format($car->price, 2) }}</span>/day</div>
                                </div>
                            @endif
                            @if (!$car->in_service)
                                <div class="text-red-500 text-xs">Currently not in service</div>
                            @endif
                            ...
                            <a href="{{ route('car.details', ['uuid' => $car->uuid]) }}"
                                class="inline-block px-5 py-2 border border-blue-500 text-blue-500 rounded-lg font-medium bg-white hover:bg-blue-500 hover:text-white transition mt-2 text-center">View
                                Details</a>
                        </div>
                    </div>
                </div>
